...
# TESTING FILE SET UP
initials <- "kek"
test_folder_loc <- paste0("./TEST_STITCH", initials,"/")
print(test_folder_loc)
if(!file.exists(test_folder_loc)){dir.create(test_folder_loc)}
```
...
stitched_L0_file <- "AvianInteractionData_L0_stitch_20251211_kek.csv" 
```
...
source(here::here('R/L0/L0_functions_TEST.R'))
...
write.csv(checked_file_list, paste0(test_folder_loc, "raw_csv_list.csv"))
```

Test reading a singlefile from the list of checked files
...
# nrow(intxnsL0sp$intxns)$intxns

write.csv(intxnsL0sp$intxns, paste0(test_folder_loc, "interactions_wide.csv"))
```

Transform 'long' format along sources from entered wide format.\
See documentation for this function in L0_functions.R
...
write.csv(intxnsL0sp$intxns, paste0(test_folder_loc, "interactions_long.csv"))
```
...
#                           stitched_L0_file,
#                           L0_dir = here::here(test_folder_loc))
# print(L0_file)
