...
L0_functions_file <- here::here('R/L0/L0_functions.R')
...
Optional: print all of the files in the review folder: 
```{r}
# species_to_review_main_names
```
...
print(paste(csv_file_path, ":", csv_file_group_name, "files to process", length(in_review_file_list)))
...
27 species' csvs that still need the 2nd checking and
...
- There are just 6 csvs in google drive that are not in species folder.  
- There many just in git not in google
- There are 293 species files in review.  

**Errors found/ Fixes to be made:**

- There is at least one file with a type in the file name:
   `Campylopterus_ hemileucurus_AJ.csv` has a space which will affect the stitch process
