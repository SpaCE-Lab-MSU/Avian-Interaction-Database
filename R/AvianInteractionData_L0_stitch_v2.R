...
# DATA INPUT:     folders of AvianInteractionDatabase csvs in L0
# DATA OUTPUT:    L1 data: AvianInteractionData_L0.csv
# PROJECT:        Avian Interaction Database & Avian Meta-Network
# DATE:           20 Mar 2023 - May 2025
# NOTES:          Next script to run: /L1/AvianInteractionData_L1.R
#
# COMBINE INDIVIDUAL CSVs into single file and reformat
# works for different directories used to track progress and
# This was written by PLZ, originally in R/L0/AvianInteractionData_L0_stitch.R
...
# this appears to be a copy of existing data.  Is it still useful or remnant of
# the Fall 2024?
google_drive_folder = "~/Google Drive/Shared drives/Avian_MetaNetwork/data/L0/avian_intxn_data/species_entry/EP"
species_dir<- file.path(L0_dir, "species")
# Directory with species that are in BBS but currently being checked by India (11/27/2024)
species_temp_dir<-file.path(L0_dir, "species_temp")
species_in_review_dir<- file.path(L0_dir, "species_in_review")
...
get_main_names_folder <- function(folder_with_csvs = "~/Google Drive/Shared drives/Avian_MetaNetwork/data/L0/avian_intxn_data/species_entry/EP" ){
  files <- list.files(folder_with_csvs, full.names = FALSE)
  main_names <- sapply(files, get_main_name)
  return(main_names )
}

# what does this do?  Is it still needed?
show_species_need_second_checks <- function(folder_with_csvs = google_drive_folder) {
  google_drive_main_names <- get_main_names_folder(folder_with_csvs )
  git_repo_main_names <- get_main_names_folder(folder_with_csvs = species_dir)
...
combine_by_species <- function(data_dir = "species_dir") {
  ...
  ### set folder variables and locations
  L0_dir <- here::here('L0')
  species_dir<- file.path(L0_dir, "species")
  # Directory with species that are in BBS but currently being checked by India (11/27/2024)
  species_temp_dir<-file.path(L0_dir, "species_temp")
...
##########
### run all the stitching and report
## see notebook file : AvianInteractionData_L0_stitch_notebooks.Rmd

main_L0_stitch<- function(data_dir = "species_dir") {
