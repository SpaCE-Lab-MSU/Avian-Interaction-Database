...
for(r in rows_to_update){
  ...
  high_confidence_match <- unique(high_confidence_matches[
    high_confidence_matches$scientific_name.raw ==current_scientific_name.raw,
    "closest_scientific_name_match"]) 
  
  if(length(high_confidence_match)>1) {
...
# this was previously use to list matches in groups but since there are so few
# just showing the whole lists sorted by confidence below 
...
#     print(select(current_range, scientific_name.raw, common_name.raw, closest_scientific_name_match,scientific_name_match_score))
#     cat("\n")
#   }
# }
...
low_confidence_matches %>% select(scientific_name.raw, closest_scientific_name_match,scientific_name_match_score) %>% arrange(desc(scientific_name_match_score))
```
...
#| echo: false
#| eval: false
# old version Save these fixes in new df
...
list those with comments that are not on checklist
```{r}

tibble(unresolved_species(int.raw.names))
...
```{r}
interaction_records_by_match(int.raw, 'hawk')
```
...
## Fixes from previous work to be confirmed
...
#### Match to checklist based on scientific name
