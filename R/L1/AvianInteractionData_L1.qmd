...
Build a list of things to updated here
...
print(nrow(int.raw.names))
print("number of species with no edits")
print(nrow(unresolved_species(int.raw.names)))
print("number of all rows with not edits including UnIDs")
...
## Merge with Checklist to see how many are GBIF-only and don't match

this adds new columns scientific_name.checklist and common_name.checklist
which would ultimately but what is published. 

scientific_name.checklist and scientific_name.edit are the same where they match
...
Count of rows Not on checklist 
```{r}
nrow(filter(int.raw.names.checklist, is.na(scientific_name.checklist)))

```

Rows to to match that are not unids: 
...
### Using curated list to match gbif to checklist

Goal: use the Canada+CONUS list which has gbif to checklist mapping
