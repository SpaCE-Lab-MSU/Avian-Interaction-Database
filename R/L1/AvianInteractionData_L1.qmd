...
AvianInteractionData_L0_file <-  "AvianInteractionData_L0_stitch_oct25.csv"
```
...
8. read in CSV of manual resolutions and apply to whole list 
9. resolve genus only unids
10. resolve hybrids ( X and / )
11. merge checklist common names into resolutions matching on scientific name
12. join checklist scientific names and common names to interactions data
13. save data files 
...
-  Oct 1, 2025: workflow reorganzation for better taxonomic resolution (PSB)
...
skipp this because the histogram shows this information 
```{r}
...
1. complete the entries in this file 
...
for(r in 1:nrow(manual_taxonomic_resolutions)){
  ...
  r = which(int.raw.names.manual$scientific_name.raw == manual.edit$scientific_name.raw)
  # TODO check if r is a real vector of integers
  int.raw.names.manual[r,]$scientific_name.edit <- manual.edit$scientific_name.raw
  int.raw.names.manual[r,]$edit_notes <- manual.edit$edit_notes
...
Update resolved common names use the checklist common names 
...
print(length(union(int.raw.resolved$species1_scientific, int.raw$species2_scientific)))


print("unique types of interactions")
print(length(unique(int.raw.resolved$interaction)))

print("unique pairwise interactions")

# this will require ignoring the order of species, so need to create pairs and sort them
# dat <- data.frame(c('a','b','c'), c('b','a','d'), stringsAsFactors=F );
# colnames(dat) <- c('V1','V2');
# x <- unique(as.data.frame(t(apply(dat, 1, sort ))));
# rownames(x) <- 1:nrow(x);
# x;
print(length(unique(paste(int.raw.resolved$species1_scientific, int.raw$species2_scientific))))
