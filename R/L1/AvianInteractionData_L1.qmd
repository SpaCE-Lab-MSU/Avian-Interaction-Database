...
taxonomic_resolutions_file_name<- here::here('R/L1', "manual_taxonomic_resolutions_L1_2025-10.csv")
...
4. use fuzzy matching to find scientific names in checklist (high confidence only)
5. resolve names common names matching checklist (high confidence only)
7. use gbif to see if name can be resolved to a checklist name
  
8. apply manual fixes and overrides to 
9. handle the genus only unids
9. save data files
...
Remaining names to be resolved
...
after applying high-confidence matches, try matching on common names from GBIF
...
if(!exists(manual_taxonomic_resolutions_file_name)) {
  warning("you must set the variable 'taxonomic_resolutions_file_name' to the path to the current file for name edits")  
    ...
    if(file.exists(manual_taxonomic_resolutions_file_name)) {
      manual_taxonomic_resolutions <- read_csv(manual_taxonomic_resolutions_file_name) 
    }
    else {
