...
unresolved_checklist_rows <- which(filter(int.raw.names.checklist, is.na(scientific_name.checklist)) %>%
  filter(!grepl(" sp\\.$", scientific_name.raw)))
  
unresolved_checklist_scinames  <- int.raw.names.checklist[unresolved_checklist_rows,] %>% 
    select(scientific_name.edit) %>% unique() 
    
print(nrow(unresoled_checklist_scinames))
