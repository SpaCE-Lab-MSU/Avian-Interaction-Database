...
4. resolve names common names matching checklist
5. use fuzzy matching to find close scientfic names to checklist
...
running TaxaDB seems to change entries for the worse in some cases, to older
versions of the taxonomy and it's not clear why. 

This is a test of how changing these entries would help or hurt. 

1) is the original (raw) already on the checklist y/n
1) does GBIF suggest a different or same s/d
2) is that GBIF suggestion on the checklist y/n
3) does the GBIF change make a checklist item no longer a checklist item
...
this is the number of entries already on the checklist that that taxadb/gbif 
makes a suggestion that is NOT on the checklist (e.g. 'breaking' the correctness)
...
#######################################################################
# STOP HERE UNTIL THE WORKFLOW IS RE-ORDERED TO MINIMIZE THIS PROBLEM #
...
It's one row of interaction so most likely remove it as an unid
...
which would ultimately be what is published. 
...
int.raw.names.checklist<- checklist_join(int.raw.names, checklist)
...
unresolved_checklist_rows <-  filter(int.raw.names.checklist, is.na(scientific_name.checklist)) %>%
  filter(!grepl(" sp\\.$", scientific_name.raw))
  
unresolved_checklist_scinames  <- unresolved_checklist_rows %>% 
    select(scientific_name.edit) %>% unique() 
    
print(nrow(unresolved_checklist_scinames))
...
suggested_update_check<- function(scientific_name.raw, scientific_name.suggested, note = NULL){
