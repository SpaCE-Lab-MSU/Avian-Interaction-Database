...
Main Species List: current conus+canada list (see code for currently use file)

Checklists:

-   current Ebird/Clements comprehensive list (see code)

-   BBS List

-   Subsetting list for current db (Canada + CONUS)

### DATA OUTPUT:

L1 data: `int.namefix.bbs.RData` (taxonomic harmonization & BBS columns into interaction data)
...
*Note that the Checklist folder above is in L0 which contains species lists, but the subset list for 2024-25 analysis is in L1 as it's build from L0 lists*
...
int.raw<-read.csv(file.path(file_paths$L0,stitched_l0_file))
print(paste("read in ", nrow(int.raw), "records from ", stitched_l0_file))
```
...
## if there is a common name need to look up
## report which rows have only commons
# remove blank rows 

#int.common_names_only<-int.raw[!with(int.raw,is.na(species1_scientific)& is.na(species2_scientific)),]
...
*These should be corrected in the L0 correct process, but catch them here before correcting taxonomy*

*TO-DO:* confirm with PLZ that these should be removed,

List rows with both missing
...
**TO-DO:** ask PLZ: most rows with missing interaction values are 'hybridization' - can we remove those?
...
**TO-DO** add this check to L0_corrections_discovery for files in review
...
*these issues should be corrected during the L0 stitching phase*
...
The following is code from when there are other species to keep, but no longer needed
...
checklists for the old and then new name
...
## SAVE CURRENT WORKSPACE WITH CURRENT DATE
...
PSB: Start on creating a data structure to hold these fixes to apply all at once

```{r}
# CHANGE TO CLOSEST MATCH:

checklist_matches<- "genus_species.raw,common_name.raw,closest_genus_species_match,genus_species_match_score,
Larus brachyrhynchos caurinus,Northwestern Crow Corvus brachyrhynchos caurinus,0.900468284
Campylorhynchos brunneicapillus,Cactus Wren Campylorhynchus brunneicapillus,0.900716846
Cathartus aura,Turkey Vulture,Cathartes aura,0.901098901
"
...
```{r}
closest_matches.df <- read.csv(text = checklist_matches, header = TRUE)
```



deal with *Branta leucopsis*
