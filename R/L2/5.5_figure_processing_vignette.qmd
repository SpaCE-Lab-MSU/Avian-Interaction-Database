...
The distribution figure (fig. 2) summarizes the whole, raw dataset. The only further step is generating a summary dataframe that counts the number of instances of each interaction type:
...
message("After initial cleaning, the data contains ", nrow(inter_NA_clean), " rows")
```
...
However, we might still have some rows where the same interaction is being represented but the rows aren't identical (different sources, etc). We will double-check this by comparing any rows that are equivalent for species1, interaction AND species2.
...
message("After removing these duplicates, we are left with ", nrow(inter_NA_dedup), " rows")
```
...
Next, we need edit `inter_NA_clean` to remove a few interaction types (co-occurrence and hybridization) that don't fit well into an ecological network, and perhaps are misrepresented across species. `inter_NA_clean`\` also contains a few duplicate interaction rows, primarily caused by the collapsing of subspecies (we prior removed self-interactions, but there are examples where one species interacts in the same way with multiple subspecies within another species, which we remove here).
