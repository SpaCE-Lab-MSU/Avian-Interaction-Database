...
c) **If your taxon is NOT an animal OR if you have already checked/entered data from BOW or ADW**:
  ...
  - vii) If the source is not located in the library, it means it is unchecked. Add it to the library. To do this, make sure you have the **Zotero Connecter** web plugin installed. Click on the plugin on the Google Scholar results page which will bring up a selection screen. Ensure you are "Saving to `MetaNetworks_SpaCELab`" and select the relevant article. This will add it and all the information directly to the Zotero Library/
